...
3. 注意事项
   ...
   3) 状态改变，没有引发重新渲染，即模板中没有使用到该数据，也会触发对应的侦听器函数
      1) 只要该状态，被侦听，无论是否引发重新渲染，只要状态改变，就会执行对应的函数
   4) 侦听函数中的this，指向当前的vue环境
      1) **所以不可以在内部修改，被该函数侦听的状态，否则会陷入死循环，报栈溢出的错误**
...
3. 父子级组件使用事件总线进行传参的注意点
   1) 父组件，在`mounted`中，添加对应的方法
   2) 子组件，在`mounted`中，立即使用对应的方法，是没有效果的
   3) 原因:
      1) 子组件的`mounted`函数先执行，然后父组件的`mounted`函数在执行
      2) 子组件，使用`$emit`触发父组件的事件函数时，该函数还没有定义，自然没有效果
      3) 找不到对应的事件函数，是不会报错的，只不过没有效果
